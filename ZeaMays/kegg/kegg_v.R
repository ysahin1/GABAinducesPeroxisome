...
gene_length_SRP014652v
length(zm_ko$gene_symbol) # annotated gene number
factor_table_all_genes <- factor(as.integer(zm_ko$gene_symbol %in% dn_SRP014652v))
length(factor_table_all_genes)
names(factor_table_all_genes) = zm_ko$gene_symbol # should be named vector
###########GOSEQ analizi####################################
length(gene_length_SRP014652v)
length(gene_length_SRP014652v[names(factor_table_all_genes)])

pwf <- nullp(factor_table_all_genes, 
             bias.data = gene_length_SRP014652v[names(factor_table_all_genes)])
...
write.table(zm_ko[ which(zm_ko$ko %in% en_KO),],file = "ko_zm_v.txt", quote = F,sep = "\t", row.names = F)
