...
perox_SRP014652v_de <- dn_SRP014652v[which(dn_SRP014652v %in% as.vector(zm_per$zm_locus))]
nperDEG <- length(perox_SRP014652v_de)
# the number of peroxisome DEGs
nperDEG
# match the symbols
perox_vstc_SRP014652v <- vstc_dds_SRP014652v[which( rownames(vstc_dds_SRP014652v) %in% perox_SRP014652v_de),]

DEperox_zscore_SRP014652v <- (perox_vstc_SRP014652v-rowMeans(perox_vstc_SRP014652v))/(rowSds(as.matrix(perox_vstc_SRP014652v)))
SRP014652v_coln <- data.frame(sample = SRP014652v_grow_CG)
...
jpeg(filename = "8vs13.jpeg", units = "in", width = 28, height = 20, res=600)
